...
const SUB_CATEGORY_RECOMMENDATIONS: Record<string, string> = {
  "token-security":
    "请使用环境变量或密钥管理服务存储令牌和密钥，勿硬编码在配置中",
  ...
  "ssrf-risk":
    "网址参数需经过严格验证和白名单过滤，禁止用户可控网址直接传入网络请求",
  "prompt-injection":
    "用户输入必须经过严格转义或结构化处理后再传入大语言模型，禁止直接拼接",
  "dangerous-syscall": "优先使用安全的接口替代命令执行，审查该调用的必要性",
  "credential-access":
    "确保访问密钥的目的正当且已获授权，勿将密钥写入日志或外部文件",
  "sensitive-path": "检查是否需要访问该敏感路径，移除不必要的路径引用",
  "dangerous-combination":
    "移除网络请求调用或限制访问目标；限制命令执行参数来源；禁止将用户输入写入可执行文件",
  typosquatting: "安装前请验证来源和作者身份，勿安装来源不明的同名扩展",
  "metadata-quality":
    "请补充完整且真实的元数据，确保至少包含说明文件或配置文件",
  "install-hook":
    "禁止在安装脚本中执行远程下载并直接执行的操作，移除可疑的安装前后钩子",
  ...
  "auth-failed":
    "认证失败，请检查认证凭证是否正确，排查是否为暴力破解或凭证泄露",
  ...
  "auth-failed-rate-limited":
    "认证请求被限流，可能是暴力破解尝试，请检查访问来源",
  "auth-failed-unknown": "未知认证失败原因，请检查 Gateway 日志",
  // 规则 9-19: 扩展检测规则处理建议
  "infrastructure-abuse":
    "请限制网络请求目标、权限范围和容器特权，移除不必要的隧道服务和数据外传端点",
  "data-exfiltration":
    "请审查数据外传目标、凭证访问和日志输出的必要性，移除对 ngrok/webhook 等服务的引用",
  "lateral-movement":
    "请审查网络扫描、隧道服务和凭证访问的必要性，限制 Docker 和 SSH 访问权限",
  "autonomy-abuse":
    "请审查远程指令获取、自修改代码和特权操作的必要性，移除免密 sudo 和动态代码执行",
  "logic-vulnerability":
    "请修复通配符导入、裸异常捕获和不安全配置，使用安全的导入和异常处理方式",
  "financial-attack":
    "请审查钱包访问、挖矿活动和资金转移逻辑，移除对可疑代币和交易信号服务的引用",
};
...
const RULE_TO_SUB_CATEGORY: Record<string, string> = {
  "token-entropy": "token-security",
  "token-weak-pattern": "token-security",
  "api-key-exposed": "token-security",
  "hardcoded-secrets": "token-security",
  "private-key-leak": "token-security",
  "mnemonic-leak": "token-security",
  "gateway-bind-exposed": "network-security",
  "tls-required": "network-security",
  "cors-wildcard": "network-security",
  "api-endpoint-safety": "network-security",
  "webhook-url-insecure": "network-security",
  "dangerous-flags": "network-security",
  "env-injection": "network-security",
  "untrusted-proxy-config": "network-security",
  "tailscale-remote-url-exposure": "network-security",
  "tailscale-enabled": "network-security",
  "tailscale-security": "network-security",
  "session-timeout-missing": "session-security",
  "session-ttl": "session-security",
  "session-isolation": "session-security",
  "session-scope-broad": "session-security",
  "gateway-auth-mode-insecure": "session-security",
  "gateway-auth-disabled": "session-security",
  "workspace-not-restricted": "data-protection",
  "log-level-verbose": "data-protection",
  "log-include-sensitive": "data-protection",
  "owner-display-info-leak": "data-protection",
  "web-search-safety": "data-protection",
  "plugins-allow-missing": "plugin-security",
  "plugin-source-untrusted": "plugin-security",
  "unverified-plugin-source": "plugin-security",
  "exec-security-disabled": "execution-security",
  "exec-permission-control": "execution-security",
  "write-no-restrictions": "execution-security",
  "deny-commands-ineffective": "execution-security",
  "dangerous-allow-commands": "execution-security",
  "native-commands-security": "execution-security",
  "native-commands-auto": "execution-security",
  "mcp-untrusted-commands": "execution-security",
  "rate-limiting": "rate-compliance",
  "npm-audit": "runtime-check",
  "node-eol": "runtime-check",
  "soul-md-injection": "runtime-check",
  "version-outdated": "runtime-check",
  "agent-defaults-security": "execution-security",
  "internal-hooks-security": "plugin-security",
  "default-port-exposed": "network-security",
  "browser-control-port": "network-security",
  "webhooks-enabled": "network-security",
  "tools-profile-permissive": "execution-security",
};

// skill_security rule → sub_category
const SKILL_RULE_TO_SUB_CATEGORY: Record<string, string> = {
  "skill-ssrf-risk": "ssrf-risk",
  "skill-prompt-injection": "prompt-injection",
  "skill-dangerous-syscall": "dangerous-syscall",
  "skill-credential-access": "credential-access",
  "skill-sensitive-path": "sensitive-path",
  "skill-dangerous-combination": "dangerous-combination",
  "skill-typosquatting": "typosquatting",
  "skill-metadata-missing": "metadata-quality",
  "skill-metadata-incomplete": "metadata-quality",
  "skill-metadata-invalid": "metadata-quality",
  "skill-dangerous-install-hook": "install-hook",
  "skill-no-engines-constraint": "permission-constraint",
  "skill-no-keywords": "permission-constraint",
  // 规则 9-19: 扩展检测规则映射
...
/**
 * 获取本机第一个非内部网络接口的 MAC 地址
 * 用于与 hostname 组合生成更可靠的机器唯一标识
 */
...
/** 缓存机器标识，避免重复计算（hostname + MAC 组合，确保唯一性） */
const MACHINE_ID: string = (() => {
 ...
 * 基于事件内容 + 机器标识生成确定性 ID
 *
 * 相同的 hostname + category + subCategory + severity + recommendation + eventInfo
 * 将始终生成相同的 ID，用于事件去重。
 * 不同终端即使事件内容相同，生成的 ID 也不同。
 */
function generateDeterministicEventId(
  ...
  const raw = `${MACHINE_ID}|${category}|${subCategory}|${severity}|${recommendation}|${eventInfo}`;
  const hash = crypto
    .createHash("sha256")
    .update(raw, "utf8")
    .digest("hex")
...
function scanResultToSecurityEvent(
  result: ScanResult,
  category: string,
  ...
  const recommendation =
    SUB_CATEGORY_RECOMMENDATIONS[subCategory] ||
    result.suggestion ||
    "请根据实际情况进行处理";
  const subCategoryDescription =
    SUB_CATEGORY_DESCRIPTIONS[subCategory] || subCategory;
  const eventInfo = [
    `规则: ${result.rule}`,
    `路径: ${result.path}`,
    `描述: ${result.message}`,
    result.currentValue !== undefined
      ? `当前值: ${JSON.stringify(result.currentValue)}`
      : "",
  ]
    .filter(Boolean)
    .join(" | ");
...
function commandViolationToSecurityEvent(
  ...
  if (
    lowerReason.includes("reverse shell") ||
    (lowerReason.includes("shell") && lowerReason.includes("/dev/tcp"))
  )
    subCategory = "reverse-shell";
  else if (
    lowerReason.includes("system destruction") ||
    lowerReason.includes("rm -rf") ||
    lowerReason.includes("mkfs")
  )
    subCategory = "system-destruction";
  else if (
    lowerReason.includes("credential") ||
    lowerReason.includes("lsass") ||
    lowerReason.includes("password")
  )
    subCategory = "credential-theft";
  else if (
    lowerReason.includes("privilege") ||
    lowerReason.includes("potato") ||
    lowerReason.includes("escalation")
  )
    subCategory = "privilege-escalation";
  else if (
    lowerReason.includes("download") ||
    (lowerReason.includes("curl") && lowerReason.includes("|"))
  )
    subCategory = "download-execute";
  else if (
    lowerReason.includes("encoding") ||
    lowerReason.includes("base64") ||
    lowerReason.includes("obfuscation")
  )
    subCategory = "encoding-obfuscation";
  else if (
    lowerReason.includes("process injection") ||
    lowerReason.includes("gdb") ||
    lowerReason.includes("ptrace")
  )
    subCategory = "process-injection";
  else if (lowerReason.includes("kernel") || lowerReason.includes("insmod"))
    subCategory = "kernel-module";
  else if (
    lowerReason.includes("remote-code-execution") ||
    lowerReason.includes("node -e") ||
    lowerReason.includes("python -c")
  )
    subCategory = "remote-code-execution";
  else if (
    lowerReason.includes("service disruption") ||
    lowerReason.includes("sshd") ||
    lowerReason.includes("stop")
  )
    subCategory = "remote-service-disruption";

  const recommendation =
    SUB_CATEGORY_RECOMMENDATIONS[subCategory] || "请立即终止该操作并排查原因";
  const subCategoryDescription =
    SUB_CATEGORY_DESCRIPTIONS[subCategory] || subCategory;
  const eventInfo = [
    `命令: ${command}`,
...
function contentCheckToSecurityEvent(
  ...
  sessionId: string,
  reqMsgId: string,
  source: "local" | "cloud" = "cloud",
): SecurityEvent {
  const isBlocked = suggestion === "block";
  const subCategory = isBlocked
    ? stage === "input"
      ? "input-blocked"
      : "output-blocked"
    : "check-passed";
  const recommendation =
    SUB_CATEGORY_RECOMMENDATIONS[subCategory] || "内容检查通过";
  const subCategoryDescription =
    SUB_CATEGORY_DESCRIPTIONS[subCategory] || subCategory;
  const eventInfo = [
...
export async function saveConfigSecurityEvents(
  source: "local" | "cloud" = "local",
): Promise<number> {
  ...
  for (const r of results) {
    const event = scanResultToSecurityEvent(r, "config_security", source);
    await dbInsertSecurityEvent(
      event.event_id,
      event.category,
      event.sub_category,
      event.sub_category_description,
      event.threat_level,
      event.event_time,
...
export async function saveRuntimeCheckEvents(
  soulResults: ScanResult[],
  npmResults: ScanResult[],
  nodeEolResults: ScanResult[],
  source: "local" | "cloud" = "local",
): Promise<number> {
  const logger = getLogger();
...
export async function saveSkillSecurityEvents(
  skillReports: Map<string, SkillScanReport>,
  source: "local" | "cloud" = "local",
): Promise<number> {
  const logger = getLogger();
  let count = 0;
  for (const [skillName, report] of skillReports) {
    for (const finding of report.findings) {
      ...
      const event = skillFindingToSecurityEvent(
        skillName,
        report.skillPath,
        finding.rule,
        finding.severity,
...
export async function saveCommandViolationEvent(
  ...
  const logger = getLogger();
  const sourceText = source === "local" ? "本地规则" : "云端检测";
  const event = commandViolationToSecurityEvent(
    command,
    reason,
    sourceText,
    source,
...
export async function saveContentCheckEvent(
  ...
  const logger = getLogger();
  const event = contentCheckToSecurityEvent(
    stage,
    content,
    suggestion,
    sessionId,
  ...
  if (suggestion === "block") {
    logger?.warn(
      `[EventStore] content_check: ${stage === "input" ? "输入" : "输出"}被拦截`,
    );
  }
}
...
export async function saveTokenUsageEvent(
  ...
  logger?.info(
    `[EventStore] token_usage: session=${sessionKey} model=${model} tokens=${totalTokens}`,
  );
}

export async function readTokenUsageEvents(
  limit: number = 100,
...
export async function saveToolCallEvent(
  ...
  logger?.info(
    `[EventStore] tool_call: tool=${toolName} success=${isSuccess} duration=${durationMs}ms session=${sessionKey}`,
  );
}

export async function readToolCallEvents(
  limit: number = 100,
...
export async function readConfigSecurityEvents(
  filter: {
    category?: string;
    subCategory?: string;
    threatLevel?: string;
...
export async function readSkillSecurityEvents(
  limit: number = 100,
): Promise<SecurityEvent[]> {
  return (await dbQuerySecurityEvents({
    category: "skill_security",
    limit,
  })) as unknown as SecurityEvent[];
}

export async function readCommandViolationEvents(
  limit: number = 100,
): Promise<SecurityEvent[]> {
  return (await dbQuerySecurityEvents({
    category: "command_violation",
...
export async function readContentCheckEvents(
  limit: number = 100,
): Promise<SecurityEvent[]> {
  return (await dbQuerySecurityEvents({
    category: "content_check",
...
export async function readGatewayAuthEvents(
  limit: number = 100,
): Promise<SecurityEvent[]> {
  return (await dbQuerySecurityEvents({
